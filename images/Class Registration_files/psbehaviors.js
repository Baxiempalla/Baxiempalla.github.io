	...
	$j.fn.makeConfReqButton = function () {
		...
		$j(this).each(function (el) {
			...
			var	text= crBtn.html(),
				...
				crCancel = $j('<button>',{id:'btnConfirmCancel', type:'button', 'class':'btnConfirmCancel'}).append(!crBtn.hasClass('ui-button')?pss_text("psx.js.scripts.psbehaviors.cancel"):pss_text("psx.js.scripts.psbehaviors.cancel")+' '+ text).addClass('hide'),// type="button" class="btnConfirmCancel">' + (!crBtn.hasClass('ui-button')?pss_text("psx.js.scripts.psbehaviors.cancel"):$j(text).prepend(pss_text("psx.js.scripts.psbehaviors.cancel")+' ').text()) + '</button>').addClass('hide'),// this button returns buttons to default state, fake delete visible, others hidden
...
$j(document).ready(function () {
	...
	$j(document).on('click','h2.toggle, h3.toggle, h4.toggle, ' +
		'th.toggle, dt.toggle', psToggle);//turns selected elements with class="toggle collapsed" or class="toggle expanded" toggle-able widget
	...
	$j(document).on('keypress','h2.toggle, h3.toggle, h4.toggle, ' +
		'th.toggle, dt.toggle', psToggle);//Enter key support - turns selected elements with class="toggle collapsed" or class="toggle expanded" toggle-able widget
	initToggle();
